...
void SEHCatcher::DoHandleDebugEvent(LPEXCEPTION_POINTERS lpEP)
{
	...
	sprintf(aBuffer,
		...
		lpEP->ContextRecord->Rax, lpEP->ContextRecord->Rbx, lpEP->ContextRecord->Rcx,
		lpEP->ContextRecord->Rdx, lpEP->ContextRecord->Rsi, lpEP->ContextRecord->Rdi);
	aDebugDump += aBuffer;
...
#ifdef _WIN64
	DWORD64 pc = theContext->Rip; 
	PDWORD64 pFrame, pPrevFrame; 

	pFrame = (PDWORD64)theContext->Rbp;
#else
